...
logger = setup_logger("evidence_retriever")
...
class EvidenceRetriever:
    def __init__(self, db: Session, history_manager: Optional[HistoryManager] = None):
        ...
        logger.info(f"Initialized EvidenceRetriever with search_limit={self.search_limit} and kpi_match_bonus={self.kpi_match_bonus}")
    ...
    def retrieve_historical_evidence(self, query: str, affected_kpis: List[str]) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(f"retrieve_historical_evidence: Searching for evidence matching query='{query}', affected_kpis={affected_kpis}")
        ...
        logger.debug(f"retrieve_historical_evidence: Enriched search query: '{enriched_query}'")

        # Search the vector index for similar past reports (config-driven limit)
        logger.debug(f"retrieve_historical_evidence: Invoking semantic_search with limit={self.search_limit}")
        ...
        for item in search_results:
            ...
            if bonus_applied:
                logger.debug(f"retrieve_historical_evidence: Applied KPI match bonus to experiment_id={exp.experiment_id}. New score={score:.4f}")
        ...
        logger.info(f"retrieve_historical_evidence: Successfully retrieved {len(evidence_list)} evidence experiments.")
