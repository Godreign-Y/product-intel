...
export async function getRecommendations() {
    ...
    if (settings.enableFastApi) {
        ...
        const optimizationPromise = fetch(`${baseUrl}/api/v1/optimization/maximize`, {
            ...
            if (!res.ok)
                throw new Error('Optimization query failed');
            return res.json();
        });
        ...
        try {
            ...
            return mockRecommendations.map(rec => {
                ...
                const dbEntry = dbRecs.find((r) => r.id === rec.id);
                ...
                if (rec.id === 'rec_1')
                    multiplier = 0.5;
                else if (rec.id === 'rec_4')
                    multiplier = 0.25;
                else if (rec.id === 'rec_5')
                    multiplier = 0.15;
                return {
                    ...rec,
...
export async function applyRecommendation(id) {
    ...
    if (settings.enableFastApi) {
        ...
        try {
            ...
            console.warn(`FastAPI apply recommendation failed. Details: ${error.message}`);
            throw error;
        }
    }
