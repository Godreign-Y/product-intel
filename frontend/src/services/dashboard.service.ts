...
import { DashboardData, KPICardData, PerformanceOverTimePoint, Anomaly, TopOpportunity, RecentExperiment } from '../types/dashboard';
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  if (settings.enableFastApi) {
    ...
    const anomalies: Anomaly[] = (declining.declining_products || []).slice(0, 3).map((item: any, idx: number) => ({
      id: `anom_db_${idx}`,
    ...
    const opportunities: TopOpportunity[] = sortedOpps.slice(0, 3).map((opp, idx) => {
    ...
    const recentExperiments: RecentExperiment[] = dbExperiments.slice(0, 5).map((exp: any) => ({
      id: exp.id,
      name: exp.name,
      status: exp.status,
